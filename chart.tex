...
\begin{figure}[htbp]
  ...
  \caption{实验设计步骤与端到端临床落地流程，从语音输入与 ASR 预处理，经信息提取与槽位填充、知识库校验，到结构化病历生成与应用场景（初诊、随访、前病史采集等）的闭环工作流。}
...
\begin{figure}[htbp]
  ...
  \caption{系统完整版本与消融版本（去除知识库纠错或改为自由生成式 LLM）的客观与主观指标对比，包括信息提取 F1 分数、文本相似度指标及幻觉发生率等。}
  \label{fig:ablation}
\end{figure}
